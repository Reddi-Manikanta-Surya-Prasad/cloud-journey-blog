...
function App() {
  ...
  const [theme, setTheme] = useState(
    () => localStorage.getItem("blog_theme_v2") || "interactive-canvas",
  );
  ...
  useEffect(() => {
    const postIdFromUrl = new URLSearchParams(window.location.search).get(
      "post",
    );
    ...
    try {
      ...
      if (data && (data.usersJson || data.listCognitoUsers?.usersJson)) {
        ...
        const mapped = parsed.map((cUser) => {
          const getAttr = (name) =>
            cUser.Attributes?.find((a) => a.Name === name)?.Value || "";
          return {
        ...
        for (const u of mapped) {
          ...
          if (!uniqueByEmail.has(key)) {
            ...
            if (
              u.status === "CONFIRMED" &&
              uniqueByEmail.get(key).status !== "CONFIRMED"
            ) {
              uniqueByEmail.set(key, u);
            }
          }
  ...
  useEffect(() => {
    ...
    setReadNotificationIds(
      JSON.parse(
        localStorage.getItem(`blog_notification_read_${currentUser.userId}`) ||
          "[]",
      ),
    );
    setDeletedNotificationIds(
    ...
    setSavedPostIds(
      JSON.parse(
        localStorage.getItem(`blog_saved_posts_${currentUser.userId}`) || "[]",
      ),
    );
    setFollowedAuthorSubs(
      JSON.parse(
        localStorage.getItem(`blog_followed_authors_${currentUser.userId}`) ||
          "[]",
      ),
    );
    setPostProgressMap(
      JSON.parse(
        localStorage.getItem(`blog_progress_${currentUser.userId}`) || "{}",
      ),
    );
    setPostReactionMap(
      JSON.parse(
        localStorage.getItem(`blog_reactions_${currentUser.userId}`) || "{}",
      ),
    );
  }, [currentUser]);
  ...
  const progressStats = useMemo(() => {
    ...
    const read = visiblePosts.filter(
      (p) => postProgressMap[p.id] === "read",
    ).length;
    const revisit = visiblePosts.filter(
      (p) => postProgressMap[p.id] === "revisit",
    ).length;
    const mastered = visiblePosts.filter(
      (p) => postProgressMap[p.id] === "mastered",
    ).length;
  ...
  const displayName =
    userAttrs.name ||
    (userAttrs.email
      ? toTitleCaseName(userAttrs.email.split("@")[0].replace(/[._-]+/g, " "))
      : "") ||
    (currentUser?.username && !/^google[_-]/i.test(currentUser.username)
  ...
  const postLikeNotifications = useMemo(() => {
    ...
    const myPostIds = new Set(
      posts
        .filter((post) => post.authorSub === currentUser.userId)
        .map((post) => post.id),
    );
    ...
    return postLikes
      .filter(
        (like) =>
          myPostIds.has(like.postId) && like.likerSub !== currentUser.userId,
      )
  ...
  const commentLikeNotifications = useMemo(() => {
    ...
    return commentLikes
      .filter(
        (like) =>
          myCommentIds.has(like.commentId) &&
          like.likerSub !== currentUser.userId,
      )
      .sort((a, b) => new Date(b.createdAt || 0) - new Date(a.createdAt || 0))
      .map((like) => {
  ...
  const followedAuthorNotifications = useMemo(() => {
    ...
    return posts
      .filter(
        (post) =>
          followed.has(post.authorSub) && post.authorSub !== currentUser.userId,
      )
      .sort((a, b) => new Date(b.createdAt || 0) - new Date(a.createdAt || 0))
      .slice(0, 25)
      .map((post) => ({
  ...
  const userBadgesBySub = useMemo(() => {
    ...
    posts.forEach((p) => {
      ...
      likesBySub.set(
        p.authorSub,
        (likesBySub.get(p.authorSub) || 0) + (p.likes?.length || 0),
      );
    });
    ...
    postsBySub.forEach((count, sub) => {
      const badges = [];
      if (count >= 5)
        badges.push({ id: "prolific", label: "Prolific Writer", icon: "âœï¸" });
      if ((likesBySub.get(sub) || 0) >= 50)
        badges.push({ id: "top", label: "Top Contributor", icon: "ðŸŽ¯" });
  ...
  const { registeredUsers, activeUsers } = useMemo(() => {
    ...
    posts.forEach((p) =>
      upsertActive(p.authorSub, p.authorName || p.authorSub),
    );
    comments.forEach((c) =>
      upsertActive(c.authorSub, c.authorName || c.authorSub),
    );
    postLikes.forEach((l) =>
      upsertActive(l.likerSub, l.likerName || l.likerSub),
    );
  ...
  async function refreshData(withSpinner = true, authModeOverride = null) {
    ...
    const readAuthMode =
      authModeOverride || (currentUser ? "userPool" : "apiKey");
    try {
      const [postRes, commentRes, commentLikeRes, postLikeRes, messageRes] =
        await Promise.all([
          ...
          readAuthMode === "userPool" && client.models.CommunityMessage
            ? client.models.CommunityMessage.list({ authMode: "userPool" })
            : { data: [] },
        ]);

      if (
        postRes.errors?.length ||
        commentRes.errors?.length ||
        commentLikeRes.errors?.length ||
        postLikeRes.errors?.length
      ) {
      ...
      if (readAuthMode === "userPool" && isAdmin) {
        try {
          ...
          if (!moderationRes.errors?.length)
            setModerations(moderationRes.data || []);
          if (!profilesRes.errors?.length)
      ...
      hydratedPosts.forEach((post) => {
        if (post.mediaPath && !cacheSnapshot[post.mediaPath])
          missingPaths.add(post.mediaPath);
      });
      ...
      hydratedPosts.forEach((post) => {
        parseContentBlocks(post.content).forEach((block) => {
          ...
          if (block.type === "html" && block.value) {
            const srcMatches = block.value.matchAll(
              /src=["'](media\/[^"']+)["']/gi,
            );
  ...
  function markNotificationsSeen() {
    ...
    localStorage.setItem(
      `blog_notification_read_${currentUser.userId}`,
  ...
  function toggleSavePost(postId) {
    ...
    localStorage.setItem(
      `blog_saved_posts_${currentUser.userId}`,
      JSON.stringify(next),
    );
  }
  ...
  function setPostProgress(postId, status) {
    ...
    localStorage.setItem(
      `blog_progress_${currentUser.userId}`,
      JSON.stringify(next),
    );
  }
  ...
  const setPostReaction = (postId, reactionCode) => {
    ...
    setPostReactionMap((prev) => {
      ...
      try {
        localStorage.setItem(
          `blog_reactions_${currentUser?.userId || "guest"}`,
          JSON.stringify(next),
        );
      } catch {
        // no-op
  ...
  function toggleFollowAuthor(authorSub) {
    if (!ensureAuth() || !authorSub || authorSub === currentUser?.userId)
      return;
    ...
    localStorage.setItem(
      `blog_followed_authors_${currentUser.userId}`,
      JSON.stringify(next),
    );
  }
  ...
  const saveProfile = async (e) => {
    ...
    try {
      ...
      const myComments = comments.filter(
        (c) => c.authorSub === currentUser.userId,
      );

      await Promise.all(
        myPosts.map((post) =>
          client.models.Post.update({ id: post.id, authorName: nextName }),
        ),
      );
      await Promise.all(
  ...
  const requestDeletion = async () => {
    ...
    try {
      const { data: profiles } = await client.models.UserProfile.list({
        filter: { userSub: { eq: currentUser.userId } },
      });
      if (profiles && profiles.length > 0) {
        await client.models.UserProfile.update({
          id: profiles[0].id,
      ...
      const postCount = posts.filter(
        (p) => p.authorSub === currentUser.userId,
      ).length;
  ...
  const submitCommunityMessage = async (e) => {
    e.preventDefault();
    if (!ensureAuth() || !newMessageSubject.trim() || !newMessageText.trim())
      return;
    try {
      await client.models.CommunityMessage.create({
        userSub: currentUser.userId,
    ...
    try {
      ...
      await Promise.all(
        commentLikes
          ...
          .map((l) =>
            client.models.CommentLike.delete(
              { id: l.id },
              { authMode: "userPool" },
            ),
          ),
      ...
      await Promise.all(
        postComments.map((c) =>
          client.models.Comment.delete({ id: c.id }, { authMode: "userPool" }),
        ),
      );
      await Promise.all(
        postLikes
  ...
  const setUserBlocked = async (userSub, blocked, reason = "") => {
    ...
    try {
      const existing = moderations.find(
        (m) => m.userSub === userSub || m.id === userSub,
      );
      if (existing) {
  ...
  const adminEditUser = async (user) => {
    alert(
      `Editing ${user.email || user.sub} requires the AdminUpdateUserAttributes IAM policy backend which is not yet deployed.`,
    );
  };
  ...
  const adminDeleteUser = async (user) => {
    ...
    const ok = window.confirm(
      `Are you sure you want to delete ${user.email}? This will SOFT delete the user by blocking them and anonymizing their posts.`,
    );
    if (!ok) return;
  ...
  const adminTriggerPasswordReset = async (user) => {
    ...
    const input = window.prompt(
      "Enter user email for password reset",
      defaultEmail,
    );
    if (!input) return;
  ...
  const togglePostLike = async (postId) => {
    ...
    try {
      const existing = postLikes.find(
        (like) =>
          like.postId === postId && like.likerSub === currentUser.userId,
      );
  ...
  const toggleCommentLike = async (postId, commentId) => {
    ...
    try {
      const existing = commentLikes.find(
        (like) =>
          like.commentId === commentId && like.likerSub === currentUser.userId,
      );
  ...
  return (
    <div className="page-shell">
          ...
          {currentUser && (
            <>
              ...
              >
                ...
                {showProfileMenu && (
                  <div
                    className="notification-panel profile-menu-panel"
                    style={{ width: "200px", right: 0, left: "auto" }}
                  >
                    <ul
                      style={{
                        listStyle: "none",
          ...
          <section className="toolbar floating-toolbar">
            ...
            {currentUser ? (
              <div className="learning-progress-strip">
                <span>
                  Completed: {progressStats.mastered}/{progressStats.total || 0}
                </span>
          ...
          <FullPostView
            ...
            onEdit={() => {
              ...
              setComposerConfig({
                ...
                topic: activePost.topic || "",
                postToDevTo: false,
                postToHashnode: false,
                postToMedium: false,
      ...
      {showDeleteWarning && (
        ...
        >
          <div
            ...
            style={{ maxWidth: "500px", border: "2px solid #ef4444" }}
          ...
          >
            ...
            <p style={{ margin: "12px 0" }}>
              ...
              erase all your progress, posts, and saved articles.
            </p>
            <p
              style={{
                fontWeight: 600,
            ...
            {authMode !== "confirm" ? (
              ...
              <>
                ...
                <input
                  value={authForm.code}
                  onChange={(e) =>
                    setAuthForm((prev) => ({ ...prev, code: e.target.value }))
                  }
                />
              </>
            )}
      ...
      {showDeleteWarning && (
        <div
          className="auth-overlay delete-warning-overlay"
          style={{ zIndex: 3000 }}
        >
          <div
            className="card modal-warning"
          ...
          >
            ...
            <p style={{ fontSize: "0.9rem", opacity: 0.8 }}>
              This action will permanently erase your posts, comments, progress,
            ...
            >
              Admin will review and process this request via: <br />
              <strong>cloudjourney@suryareddi.in</strong>
            </p>
            <div className="button-row" style={{ marginTop: "24px" }}>
              <button className="danger" onClick={requestDeletion}>
