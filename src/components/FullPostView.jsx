...
export default function FullPostView({
  ...
  const readMinutes = useMemo(
    () => estimateReadMinutes(post.content),
    [post.content],
  );
  ...
  const depthSummary =
    depthMode === "pro"
      ? post.proSummary || post.beginnerSummary || ""
      : post.beginnerSummary || post.proSummary || "";
  ...
  const handleListenToggle = () => {
    ...
    const speakNow = () => {
      const plainTitle = stripReadableText
  ...
  return (
    <article className="card full-post" ref={postRef}>
      ...
      <div className="post-head">
        <div>
          ...
          >
            ...
            <div
              className={`skill-pill ${skill.cls}`}
              style={{ marginLeft: 4 }}
            >
      ...
      <div className="full-post-content">
        ...
        >
          <small
            style={{ opacity: 0.6, fontWeight: 500, letterSpacing: "0.03em" }}
          >
            EXPLAIN MODE
          </small>
          {canInteract ? (
            <div
              style={{
                display: "flex",
                background: "var(--bg-shell,#f0f0f0)",
                borderRadius: 99,
                padding: 3,
                gap: 2,
              }}
            >
              ...
              <button
                ...
                style={{
                  ...
                  boxShadow:
                    depthMode === "pro" ? "0 2px 8px rgba(0,0,0,0.3)" : "none",
                }}
              >
                ⚫ Pro
              </button>
        ...
        {(() => {
          const isOnline =
            post.authorSub.charCodeAt(0) % 2 === 0 ||
        ...
        {post.tldr ? (
          <section
            style={{
              background:
                "linear-gradient(135deg, rgba(79,172,254,0.12), rgba(0,242,254,0.08))",
        ...
        {post.whyMatters ? (
          <section
            style={{
              background:
                "linear-gradient(135deg, rgba(251,191,36,0.1), rgba(245,158,11,0.07))",
        ...
        {contentBlocks.map((block, index) => {
          ...
          if (block.type === "mermaid") {
            return (
              <MermaidBlock
                key={`${post.id}-full-mermaid-${index}`}
                code={block.code}
              />
            );
          }
          if (block.type === "code") {
            return (
              <CodeBlock
                key={`${post.id}-full-code-${index}`}
                code={block.code}
                lang={block.lang}
              />
            );
          }
          return renderStyledTextBlock(
            block.value,
            `${post.id}-full-text-${index}`,
          );
        })}
        ...
        <div className="phase2-row">
          <div className="phase2-block">
            ...
            <div className="button-row">
              <button
                className="ghost"
                onClick={() => onSetProgress?.(post.id, "read")}
              >
                {"\u2705"} Read
              </button>
              <button
                className="ghost"
                onClick={() => onSetProgress?.(post.id, "revisit")}
              >
                {"\u{1F516}"} Revisit
              </button>
              <button
                className="ghost"
                onClick={() => onSetProgress?.(post.id, "mastered")}
              >
                {"\u2B50"} Mastered
              </button>
            </div>
      ...
      {!canInteract ? (
        <p className="guest-action-hint">
          Login to like, comment, and save posts.
        </p>
      ) : null}
                  ...
                  {openCommentMenuId === comment.id ? (
                    <div className="comment-menu">
                      ...
                      <button
                        ...
                        title={
                          mine ? "Edit comment" : "Only comment owner can edit"
                        }
                        onClick={() => {
