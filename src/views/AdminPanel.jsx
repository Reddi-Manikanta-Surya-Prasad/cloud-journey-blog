export default function AdminPanel({
  ...
  return (
    <section className="card admin-panel">
      ...
      >
        <button
          className={`ghost ${adminTab === "users" ? "active-tab" : ""}`}
          onClick={() => setAdminTab("users")}
        >
          Users &amp; Stats
        </button>
        <button
          className={`ghost ${adminTab === "messages" ? "active-tab" : ""}`}
      ...
      {adminTab === "users" ? (
        <div className="admin-section">
                  ...
                  return (
                    <tr key={`reg-${u.sub}`}>
                      ...
                      <td className="admin-actions">
                        ...
                        <button
                          className="ghost"
                          onClick={() => adminTriggerPasswordReset(u)}
                        >
                          Password Reset
                        </button>
                        {blocked ? (
                          <button
                            className="ghost"
                            onClick={() => setUserBlocked(u.sub, false)}
                          >
                            Unblock
                          </button>
                        ) : (
          ...
          <h3 style={{ marginTop: "30px" }}>
            Active Content Creators (Posts/Comments/Likes)
          </h3>
          <div className="table-wrapper">
            <table className="admin-table">
              <thead>
                      ...
                      <td className="admin-actions">
                        ...
                        {blocked ? (
                          ...
                          <button
                            className="danger"
                            onClick={() =>
                              setUserBlocked(u.sub, true, "Blocked by admin")
                            }
                          >
                            Block
                          </button>
            ...
            <table className="admin-table">
              ...
              <tbody>
                {comments.map((c) => (
                  <tr key={c.id}>
                    <td>
                      {posts.find((p) => p.id === c.postId)?.title || "Post"}
                    </td>
                    <td>{c.text}</td>
                    <td>{c.authorName}</td>
                    <td>
                      ...
                      <button
                        ...
                        onClick={async () => {
                          if (window.confirm("Delete this message?")) {
                            await client.models.CommunityMessage.delete({
                              id: msg.id,
                            });
                            refreshData();
                          }
